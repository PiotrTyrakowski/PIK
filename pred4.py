...
shares = 0
capital = cash
bad_purchase = False  # flag to indicate whether the previous purchase was not profitable
...
for i in range(len(second_90_days) - 60):  # Subtract 60 to avoid going beyond the data
    ...
    if future_price > current_price:
        ...
        # If the previous purchase was not profitable, buy more shares
        if bad_purchase:
            buy_shares *= 2

        # Buy shares and update cash and shares
        shares += buy_shares
    ...
    # Check if the previous purchase was profitable
    if shares > 0 and current_price > (cash / shares):
        bad_purchase = False
    else:
        bad_purchase = True
